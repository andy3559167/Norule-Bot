...
const quickSettings = [
  { tab: 'general' as DashboardTab, icon: '⌂', tone: 'violet', title: '一般設定', description: '管理語言與這個伺服器的基礎偏好。' },
    ...
    <template v-else-if="!dashboard.state.authenticated">
      <main class="nr-auth-page">
        <section class="nr-auth-card">
          <div class="nr-brand is-centered"><span class="nr-brand-mark"><img v-if="dashboard.state.bot?.avatarUrl" :src="dashboard.state.bot.avatarUrl" :alt="dashboard.state.bot.username" referrerpolicy="no-referrer"><template v-else>N</template><i /></span><span><strong>{{ dashboard.state.bot?.username || 'NoRule' }}</strong><small>BOT CONSOLE</small></span></div>
        ...
        <main class="nr-content">
          <template v-if="dashboard.manageableGuilds.value.length && dashboard.state.selectedGuildId">
            <header class="nr-page-head">
              <div><p class="nr-breadcrumb"><button type="button" :aria-current="dashboard.state.atHome ? 'page' : undefined" @click="dashboard.openHome">控制台</button><template v-if="!dashboard.state.atHome"> / <span>{{ activeMeta.title }}</span></template></p><h1>{{ dashboard.state.atHome ? '控制台首頁' : activeMeta.title }}</h1><h2>{{ dashboard.state.atHome ? '集中掌握伺服器狀態，快速前往常用管理功能。' : activeMeta.description }}</h2></div>
              <div v-if="dashboard.state.atHome" class="nr-page-head-actions"><button type="button" class="nr-button is-secondary" :disabled="dashboard.state.loadingGuilds" @click="dashboard.loadGuilds">↻ {{ dashboard.state.loadingGuilds ? '重新整理中…' : '重新整理伺服器' }}</button></div>

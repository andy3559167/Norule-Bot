...
public final class FreeDictionaryApiGateway implements DictionaryApiGateway {
    ...
    public FreeDictionaryApiGateway(HttpClient httpClient, String endpoint, Duration requestTimeout) {
        this(httpClient, true, endpoint, requestTimeout);
    }

    private FreeDictionaryApiGateway(
            HttpClient httpClient,
            boolean enabled,
            String endpoint,
            Duration requestTimeout
    ...
    public CompletableFuture<DictionaryLookupResult> lookup(String word) {
        ...
        try {
            ...
            LOGGER.warn("Free Dictionary lookup request could not be created: word={}", safeWord);
        ...
        try {
            ...
            LOGGER.warn("Free Dictionary lookup failed to start: word={}", safeWord);
            return CompletableFuture.completedFuture(DictionaryLookupResult.API_ERROR);
        }
    }
    ...
    private DictionaryLookupResult resolveResponse(
        ...
        if (error != null || response == null) {
            LOGGER.warn(
                    "Free Dictionary lookup failed: word={}, cause={}",
                    word,
        ...
        if (status < 200 || status >= 300) {
            LOGGER.warn("Free Dictionary lookup failed: word={}, status={}", word, status);
            return DictionaryLookupResult.API_ERROR;
        }
        ...
        try {
            ...
            LOGGER.warn("Free Dictionary returned invalid JSON: word={}", word);
            return DictionaryLookupResult.API_ERROR;
        }
    }

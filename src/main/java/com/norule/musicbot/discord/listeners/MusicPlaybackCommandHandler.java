...
final class MusicPlaybackCommandHandler {
    ...
    MusicPlaybackCommandHandler(MusicCommandListener owner) {
        this.owner = owner;
    }
    ...
    void handleVolumeSlash(SlashCommandInteractionEvent event, String lang) {
        ...
        if (volumeOption == null) {
            volumeOption = event.getOption(MusicCommandListener.OPTION_VOLUME_VALUE_ZH);
        }
    ...
    void handleJoinSlash(SlashCommandInteractionEvent event) {
    ...
    void handleStopSlash(SlashCommandInteractionEvent event) {
    ...
    void handleLeaveSlash(SlashCommandInteractionEvent event) {
    ...
    void handleRepeatSlash(SlashCommandInteractionEvent event, String lang) {
    ...
    void handleTextCommand(MessageReceivedEvent event, Guild guild, String cmd, String arg, String lang) {
        switch (cmd) {
    ...
    void handlePlayPick(StringSelectInteractionEvent event, String lang) {
    ...
    private void directPlay(Guild guild, Member member, String query, MusicCommandListener.TextSink sink, TextChannel panelChannel) {
    ...
    private void handleJoin(Guild guild, Member member, MusicCommandListener.TextSink sink) {
        String lang = owner.lang(guild.getIdLong());
        if (member == null || member.getVoiceState() == null || member.getVoiceState().getChannel() == null) {
            sink.send(owner.i18nService().t(lang, "music.join_first"));
            return;
    ...
    private void handleSkip(Guild guild, MusicCommandListener.TextSink sink) {
        String lang = owner.lang(guild.getIdLong());
    ...
    private void handleStop(Guild guild, MusicCommandListener.TextSink sink) {
        String lang = owner.lang(guild.getIdLong());
        if (guild.getAudioManager().getConnectedChannel() == null) {
            sink.send(owner.i18nService().t(lang, "music.not_connected"));
            return;
    ...
    private void handleLeave(Guild guild, MusicCommandListener.TextSink sink) {
        String lang = owner.lang(guild.getIdLong());
        if (guild.getAudioManager().getConnectedChannel() == null) {
            sink.send(owner.i18nService().t(lang, "music.not_connected"));
            return;
    ...
    private String getPlayQuery(SlashCommandInteractionEvent event) {
        ...
        if (queryOption == null) {
            queryOption = event.getOption(MusicCommandListener.OPTION_QUERY_ZH);
        }

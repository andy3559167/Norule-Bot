...
public final class GuildSettingsWebService {
    ...
    private void handleTicketRoute(HttpExchange exchange, Guild guild, String[] segments) throws IOException {
        ...
        if ("GET".equalsIgnoreCase(exchange.getRequestMethod()) && "transcript".equals(action)) {
            ...
            ticketTranscriptWebService.handleTicketTranscriptDownload(exchange, guild, segments[3]);
            return;
        }
        if ("DELETE".equalsIgnoreCase(exchange.getRequestMethod()) && "transcript".equals(action)) {
            ...
            ticketTranscriptWebService.handleTicketTranscriptDelete(exchange, guild, segments[3]);
            return;
        }
    ...
    private DataObject buildWelcomePayload(GuildSettingsService.GuildSettings settings) {
        var welcome = settings.getWelcome();
        return DataObject.empty()
    ...
    private DataObject buildPrivateRoomPayload(GuildSettingsService.GuildSettings settings) {
        var privateRoom = settings.getPrivateRoom();
        return DataObject.empty()
    ...
    void handleSettingsSave(HttpExchange exchange, Guild guild) throws IOException {
        ...
        GuildSettingsService.GuildSettings updated = owner.settingsService().updateSettings(guild.getIdLong(), current -> {
            ...
            var n = current.getNotifications();
            ...
            if (languageChanged) {
                if (owner.shouldAutoApplyTemplate(nMap, "memberJoinMessage", current.getNotifications().getMemberJoinMessage())) {
                    n = n.withMemberJoinMessage(owner.i18n().t(language, "notifications.template.default.member_join"));
                }
                if (owner.shouldAutoApplyTemplate(nMap, "memberLeaveMessage", current.getNotifications().getMemberLeaveMessage())) {
                    n = n.withMemberLeaveMessage(owner.i18n().t(language, "notifications.template.default.member_leave"));
                }
                if (owner.shouldAutoApplyTemplate(nMap, "voiceJoinMessage", current.getNotifications().getVoiceJoinMessage())) {
                    n = n.withVoiceJoinMessage(owner.i18n().t(language, "notifications.template.default.voice_join"));
                }
                if (owner.shouldAutoApplyTemplate(nMap, "voiceLeaveMessage", current.getNotifications().getVoiceLeaveMessage())) {
                    n = n.withVoiceLeaveMessage(owner.i18n().t(language, "notifications.template.default.voice_leave"));
                }
                if (owner.shouldAutoApplyTemplate(nMap, "voiceMoveMessage", current.getNotifications().getVoiceMoveMessage())) {
                    n = n.withVoiceMoveMessage(owner.i18n().t(language, "notifications.template.default.voice_move"));
                }
            }

            var l = current.getMessageLogs();
            ...
            if (!mMap.isEmpty()) {
                m = m.withAutoLeaveEnabled(owner.boolOrDefault(mMap, "autoLeaveEnabled", m.isAutoLeaveEnabled()))
                        ...
                        .withDefaultRepeatMode(parseRepeatMode(owner.stringOrDefault(mMap, "defaultRepeatMode", m.getDefaultRepeatMode().name()), m.getDefaultRepeatMode()))
            ...
            var t = current.getTicket();
            Map<String, Object> tMap = owner.asMap(root.get("ticket"));
            if (!tMap.isEmpty()) {
                var options = parseTicketOptions(tMap, t.getOptions());
                t = t.withEnabled(owner.boolOrDefault(tMap, "enabled", t.isEnabled()))
                        ...
                        .withOpenUiMode(parseTicketOpenUiMode(owner.stringOrDefault(tMap, "openUiMode", t.getOpenUiMode().name()), t.getOpenUiMode()))
    ...
    private <E extends Enum<E>> E parseRepeatMode(String raw, E fallback) {
        if (fallback == null) {
            return null;
        }
        try {
            E parsed = Enum.valueOf(fallback.getDeclaringClass(), raw.trim().toUpperCase());
            return parsed;
    ...
    private <E extends Enum<E>> E parseTicketOpenUiMode(String raw, E fallback) {
        if (fallback == null) {
            return null;
        }
        ...
        if ("BUTTONS".equals(normalized)) {
            normalized = "BUTTON";
        }
        try {
            E parsed = Enum.valueOf(fallback.getDeclaringClass(), normalized);
            return parsed;
        } catch (Exception ignored) {
            return fallback;
        }
    ...
    private <T> List<T> parseTicketOptions(Map<String, Object> map, List<T> fallback) {
        ...
        for (Object item : iterable) {
            Map<String, Object> optionMap = owner.asMap(item);
            if (optionMap.isEmpty()) {
                continue;
            }
            T defaultOption = fallback != null && fallback.size() > index
                    ? fallback.get(index)
                    : (fallback == null || fallback.isEmpty() ? null : fallback.get(0));
            if (defaultOption == null) {
                continue;
            }
            try {
                var fromMap = defaultOption.getClass().getMethod("fromMap", Map.class, defaultOption.getClass());
                @SuppressWarnings("unchecked")
                T built = (T) fromMap.invoke(null, optionMap, defaultOption);
                if (built != null) {
                    out.add(built);
                }
            } catch (Exception ignored) {
